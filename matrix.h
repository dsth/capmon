...
// really lazy, just using a recipe from cookbook?
#include "stride_iter.h" // see Recipe 11.12
#include <valarray>
#include <numeric>
#include <algorithm>

template<class Value_T> class matrix {
  ...
  public:
    ...
    matrix(int r, int c) : m(r * c), nrows(r), ncols(c) { }
    matrix(const self& x) : m(x.m), nrows(x.nrows), ncols(x.ncols) { }

    template<typename T> explicit matrix(const std::valarray<T>& x) : m(x.size( ) + 1), nrows(x.size( )), ncols(1) {
        for (int i=0; i<x.size( ); ++i) m[i] = x[i];
    }

    template<typename T> explicit matrix(const matrix<T>& x) : m(x.size( ) + 1), nrows(x.nrows), ncols(x.ncols) {
    ...
    self& operator=(const self& x) { m = x.m; nrows = x.nrows; ncols = x.ncols; return *this; }
